            ...
            <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} onViewportEnter={() => setCelebrate(true)} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 10 }}>
                    {boxes.map((item, index) => (
                        <motion.div key={item.label} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ delay: index * 0.08, duration: 0.6 }} style={{ padding: '16px 6px', border: '1px solid rgba(201,162,75,0.35)', background: 'rgba(201,162,75,0.04)' }}>
                            <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(1.6rem, 5.5vw, 2.1rem)', fontWeight: 600, color: 'var(--gold-light)' }}>{item.value}</div>
